...
def build_vision_encoder():
    ...
    if vision_config.load_vision_pretrained:
        if os.path.exists(vision_config.vision_model_weights):
            vison_encoder.load_state_dict(torch.load(vision_config.vision_model_weights, map_location='cpu'), strict=False)
...
def build_text_encoder():
    ...
    if text_config.load_text_pretrained:
        if os.path.exists(text_config.text_model_weights):
            text_encoder.load_state_dict(torch.load(text_config.text_model_weights, map_location='cpu'), strict=False)
