...
class ImageTextModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if token_type_ids is None:
            if hasattr(self.text_embedding, "token_type_ids"):
                buffered_token_type_ids = self.text_embedding.token_type_ids[
                    :, :seq_length
                ]
        ...
        if self.config.is_decoder:
            extended_attention_mask = __class__.correct_extend_attention_mask(
    ...
    @staticmethod
    def correct_extend_attention_mask(
        ...
        if triangle_mask.shape[1] == attention_mask_[:, :-num_patches_].shape[1]:
            # Filtering ...
            filtered_triangle_mask = (
                triangle_mask * attention_mask_[:, :seq_length][:, None, :]
            )

            # Create full attention mask for text modality to image modality
            text_to_image_mask = torch.ones(
                (batch_size, seq_length, num_patches_), dtype=dtype, device=device
            )
            triangle_text_to_image_mask = torch.concat(
                (filtered_triangle_mask, text_to_image_mask), dim=-1
            )

            # Create extended_attention_mask with zeros as placeholders
            extended_attention_mask = torch.zeros(
                (batch_size, seq_length + num_patches_, seq_length + num_patches_),
                dtype=dtype,
                device=device,
            )
            extended_attention_mask[:, :seq_length, :].copy_(
                triangle_text_to_image_mask
            )
            # Filtering ...
            extended_attention_mask = (
                extended_attention_mask.transpose(-1, -2) * attention_mask_[:, None, :]
            ).transpose(
                -1, -2
            )  # noqa
            ...
            extended_attention_mask[:, seq_length:, seq_length:].copy_(
                torch.ones(
                    (batch_size, num_patches_, num_patches_), dtype=dtype, device=device
                )
            )
        else:
            ...
            prefix_seq_len = (
                attention_mask_[:, :-num_patches_].shape[1] - triangle_mask.shape[1]
            )
            ...
            triangle_text_to_image_mask = torch.concat(
                (triangle_mask, text_to_image_mask), dim=-1
            )
            ...
            extended_attention_mask[
                :, seq_length:, seq_length + prefix_seq_len :
            ].copy_(
                torch.ones(
                    (batch_size, num_patches_, num_patches_), dtype=dtype, device=device
                )
...
class ImageTextForPretraining(RobertaPreTrainedModel):
    ...
    def __init__(self, config: ImageTextConfig):
        ...
        if config.is_decoder:
            logging.warning(
                "If you want to use `RobertaForMaskedLM` make sure `config.is_decoder=False"
            )
    ...
    def forward(
        ...
        if labels is not None and match_labels is not None:
            ...
            next_sentence_loss = loss_fct(
                imagetext_relations_score.view(-1, 2), match_labels.view(-1)
            )
    ...
    def prepare_inputs_for_generation(
        ...
        if past is not None:
            ...
            assert type(self.__image_input_cache__) is not None, (
                "self.__image_input_cache is None. "
            ...
            extra_attention_mask = torch.ones(
                batch_length, num_patches, dtype=attention_mask.dtype
            )
            ...
            self.__image_input_cache__ = model_kwargs.get("image_input", None).repeat(
